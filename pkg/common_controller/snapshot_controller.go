...
func (ctrl *csiSnapshotCommonController) syncContent(content *crdv1.VolumeSnapshotContent) error {
	...
	// AnnShouldDelete annotation on the content.
	// This will trigger the deletion of the content in the
	// sidecar controller.
...
func (ctrl *csiSnapshotCommonController) syncSnapshot(snapshot *crdv1.VolumeSnapshot) error {
	...
	if content != nil && content.Spec.DeletionPolicy == crdv1.VolumeSnapshotContentDelete {
		klog.V(5).Infof("syncSnapshot: Content [%s] deletion policy [%s] is delete.", content.Name, content.Spec.DeletionPolicy)
		deleteContent = true
	}
	...
	if utils.IsSnapshotDeletionCandidate(snapshot) {
		...
		if content != nil && deleteContent {
			...
			if err != nil {
				...
				return fmt.Errorf("failed to delete VolumeSnapshotContent %s from API server: %q", *snapshot.Status.BoundVolumeSnapshotContentName, err)
...
func (ctrl *csiSnapshotCommonController) syncUnreadySnapshot(snapshot *crdv1.VolumeSnapshot) error {
	...
	if snapshot.Spec.Source.VolumeSnapshotContentName != nil {
		...
		if contentObj := ctrl.getMatchSnapshotContent(snapshot); contentObj != nil {
			...
			if !found {
				// TODO(xyang): this is not an error now because
				...
				klog.V(5).Infof("before converting content object for snapshot %s", uniqueSnapshotName)
				_, ok := contentObj.(*crdv1.VolumeSnapshotContent)
				if !ok {
					return fmt.Errorf("expected volume snapshot content, got %+v", contentObj)
				}
				klog.V(5).Infof("after converting ontent object for snapshot %s", uniqueSnapshotName)
			}
			...
			if snapshot.Spec.Source.PersistentVolumeClaimName == nil {
				...
				// TODO(xyang): Should we set BoundVolumeSnapshotContentName when content
				// was first created without status here? Actually the following loop
				// won't update snapshot status because content.Status is nil here
...
func (ctrl *csiSnapshotCommonController) ensureSnapshotSourceFinalizer(snapshot *crdv1.VolumeSnapshot) error {
...
func (ctrl *csiSnapshotCommonController) removeSnapshotSourceFinalizer(snapshot *crdv1.VolumeSnapshot) error {
	// Get snapshot source which is a PVC
	pvc, err := ctrl.getClaimFromVolumeSnapshot(snapshot)
	if err != nil {
...
func (ctrl *csiSnapshotCommonController) isSnapshotSourceBeingUsed(snapshot *crdv1.VolumeSnapshot) bool {
	klog.V(5).Infof("Checking isSnapshotSourceBeingUsed for snapshot [%s]", utils.SnapshotKey(snapshot))
	// Get snapshot source which is a PVC
	pvc, err := ctrl.getClaimFromVolumeSnapshot(snapshot)
	if err != nil {
...
func (ctrl *csiSnapshotCommonController) checkandRemoveSnapshotSourceFinalizer(snapshot *crdv1.VolumeSnapshot) error {
	...
	if slice.ContainsString(pvc.ObjectMeta.Finalizers, utils.PVCFinalizer, nil) {
		...
		inUse := ctrl.isSnapshotSourceBeingUsed(snapshot)
		if !inUse {
...
// "desired"
func (ctrl *csiSnapshotCommonController) shouldProvision(snapshot *crdv1.VolumeSnapshot) (bool, error) {
...
func (ctrl *csiSnapshotCommonController) bindandUpdateVolumeSnapshot(snapshotContent *crdv1.VolumeSnapshotContent, snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	if snapshotContent.Status != nil {
		// Update snapshot status with creationTime
		for i := 0; i < ctrl.createSnapshotContentRetryCount; i++ {
			klog.V(5).Infof("syncUnreadySnapshot [%s]: trying to update snapshot status", utils.SnapshotKey(snapshotCopy))
...
func (ctrl *csiSnapshotCommonController) updateSnapshotStatus(snapshot *crdv1.VolumeSnapshot, content *crdv1.VolumeSnapshotContent) (*crdv1.VolumeSnapshot, error) {
	...
	if content.Status == nil {
		// content status for snapshot is nil. Nothing to update yet
		return nil, nil
	}
...
func (ctrl *csiSnapshotCommonController) contentExists(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshotContent, error) {
	...
	content, ok := obj.(*crdv1.VolumeSnapshotContent)
	if !ok {
		return content, fmt.Errorf("Cannot convert object from snapshot content store to VolumeSnapshotContent %q!?: %#v", *snapshot.Status.BoundVolumeSnapshotContentName, obj)
	}
...
func (ctrl *csiSnapshotCommonController) verifySnapshotClass(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	klog.V(5).Infof("verifySnapshotClass: VolumeSnapshotClass verified: [%+v]", class)

	// Common snapshot controller does not have a snapshotterName
	// because it is deployed on the Kubernetes master node and
	// can be handling snapshot requests for multiple CSI drivers
